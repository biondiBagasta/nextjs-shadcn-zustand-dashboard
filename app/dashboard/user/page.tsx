...
export interface UserFormsControl {
  username: string;
  password: string;
  full_name: string;
  address: string
...
export default function UserPage() {
  const [formControl, setFormControl] = useState<UserFormsControl>({
    username: "",
    password: "",
    full_name: "",
    address: "",
  ...
  const initialize = () => {
    ...
    const initializeSubscription = userService.findMany().pipe(
      tap(data => {
        setDataList(data);
  ...
  const updateFormAndSelectedData = (data: UserData) => {
    setFormControl({
  ...
  const createData = () => {
    ...
    if(formValidity == false) {
      ...
      const createSubscription = userService.create({
        username: formControl.username,
        password: formControl.password,
        full_name: formControl.full_name,
  ...
  const updateData = () => {
    ...
    if(formValidity == false) {
      ...
      setIsLoadingSubmit(true);
      const updateSubscription = userService.update(selectedData!.id, {
        username: formControl.username,
        password: formControl.password,
        full_name: formControl.full_name,
        address: formControl.address,
  ...
  const deleteData = () => {
    ...
    const deleteSubscription = userService.delete(selectedData!.id).pipe(
      tap((response) => {
        initialize();
  ...
  return (
    <>
      <div className="section">
        <PageTitleComponent title="User" subtitle="Dashboard" />
        <div className="mt-5">
          <Card>
            <CardHeader>
              ...
              <CardAction>
                ...
                title="Add User" trigger={
                  ...
                  <UserForms control={ formControl } setControl={ setFormControl } />
                        ...
                        <TableCell>
                          <div className="flex flex-col md:flex-row gap-2">
                            ...
                            isLoadingSubmit={ isLoadingSubmit }
                              ...
                              description="Are you sure want to delete this User Data???" 
                              onSubmit={
                                deleteData
                              }
                              trigger={
